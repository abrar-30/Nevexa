// Enhanced API configuration with better cross-domain support
...
// Retry mechanism
async function withRetry<T>(operation: () => Promise<T>, maxRetries = 2, delay = 500): Promise<T> {
  let lastError: Error
...
async function apiRequest<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  return withRetry(operation, 2, 500)
}
